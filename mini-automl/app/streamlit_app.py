...
# Quiet TensorFlow's C++ logging before tensorflow gets imported anywhere
# downstream (via automl.ann_builder), so the terminal/deploy logs stay clean.
os.environ.setdefault("TF_CPP_MIN_LOG_LEVEL", "3")

# Make both `automl` (in src/) and `app.components` (at repo root) importable
# without requiring `pip install -e .`. The Dockerfile happens to run that
# install step, but most non-Docker hosts (e.g. Streamlit Community Cloud)
# only run `pip install -r requirements.txt` and never install this repo's
# own package -- so we add both roots to sys.path directly rather than
# relying on an editable install being present.
