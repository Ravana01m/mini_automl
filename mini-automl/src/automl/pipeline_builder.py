...
class AutoMLPipeline:
    ...
    def run(
        ...
        # Hold out a test split for an honest final evaluation (deep-dive charts)
        ...
        X_processed = self.preprocessing_pipeline_.fit_transform(X_train, y_train)
        X_test_processed = self.preprocessing_pipeline_.transform(X_test)
        logger.info(
            "Preprocessing complete: %s -> %s",
            X_train.shape, X_processed.shape,
        )
        ...
        self.leaderboard_ = self.trainer_.train_and_evaluate(
            X_processed, y_train, models, progress_callback=None,
        )
        ...
        self.leaderboard_ = self.trainer_.grid_search(X_processed, y_train, models)
        ...
        if not skip_optuna:
            ...
            self.leaderboard_ = self.trainer_.optuna_tune(
                X_processed, y_train, top_n=2, n_trials=50,
            )
