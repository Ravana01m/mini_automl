...
class Trainer:
    ...
    def get_leaderboard(self) -> pd.DataFrame:
        """Build the current leaderboard from self.results_, ranked best-first.

        Successful models are ranked by the primary metric (f1_weighted_mean
        for classification, r2_mean for regression, both higher-is-better).
        Failed models sink to the bottom.
        """
        ...
        if metric_col in leaderboard.columns:
            ...
            leaderboard = leaderboard.loc[
                rank_key.sort_values(ascending=False).index
            ...
            try:
                ...
                for res in self.results_:
                    if res["model"] == name:
                        ...
                        elif "r2" in self.scoring_metric:
